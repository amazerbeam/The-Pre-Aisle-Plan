...
### Login Component
```javascript
// LoginButton.js - OAuth login buttons
const LoginButton = () => {
  const { login } = useAuth();

  return (
    <div className="login-buttons">
      <button onClick={() => login('google')} className="btn-oauth btn-google">
        Continue with Google
      </button>
      <button onClick={() => login('github')} className="btn-oauth btn-github">
...
2. **JWT Secret**: Strong, environment-variable stored secret
