...
CREATE TABLE recipes (
  ...
  INDEX idx_meal_types ((CAST(meal_types AS CHAR(100) ARRAY))),
...
-- Prevent modifications to audit log (trigger)
DELIMITER //
...
### Archived Meal Plans Table (for 6-month retention)
```sql
...
```sql
-- Log recipe change (called via application)
...
## Data Migration (from recipes.js)

```sql
-- Sample migration script for existing recipes
INSERT INTO recipes (id, name, meal_types, default_servings, calories, ingredients, steps, is_cheat)
VALUES
  (1, 'Porridge', '["breakfast"]', 2, 880,
   '[{"name":"Rolled oats","quantity":80,"unit":"g"},{"name":"Milk","quantity":400,"unit":"ml"}]',
   '["Combine oats and milk in pot","Cook on medium heat for 5 minutes","Stir occasionally"]',
   FALSE),
  -- ... more recipes
;
```
