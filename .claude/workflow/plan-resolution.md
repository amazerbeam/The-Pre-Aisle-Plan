...
.claude/contract/
  <slug>/                  one folder per plan
    ...
    spec.md                present only when /fb-plan consumed a /brainstorming spec
  specs/                   /brainstorming output awaiting a plan
...
jira-key    := [A-Z][A-Z0-9]+ "-" [0-9]+          e.g. FB-142
...
Prefer the Jira key when the subtask has one; fall back to the planning date. The date prefix sorts chronologically, so the newest plan is last in a directory listing.
