...
Drive a focused dialogue that turns a half-formed idea into a written, user-approved spec under `.claude/contract/specs/`. The spec is the upstream artefact for `/fb-plan` — this skill never invokes `/fb-plan` itself.

## When to Use This Skill

- The user describes a new feature, refactor, or behaviour change without a written design.
- A request exists but is too vague for `/fb-plan` to produce a useful contract.
...
- Skip when the user provides a fully-specified request and explicitly asks to go straight to `/fb-plan`.

## Relationship to `/fb-plan`

```
idea ──► /brainstorming ──► .claude/contract/specs/<date>-<topic>-spec.md ──► /fb-plan
```

This skill produces the **spec**. `/fb-plan` consumes the spec file and produces the **contract** — a plan folder at `.claude/contract/<slug>/` holding `plan.md` + `tasks.md`, with the consumed spec moved in as `spec.md`. The two skills do not call each other; the user is the integrator.

<HARD-GATE>
Do not invoke any implementation skill, write any production code, scaffold any module, or invoke `/fb-plan` until the spec has been written to `.claude/contract/specs/` and the user has explicitly approved it. This applies to every change regardless of perceived simplicity.
</HARD-GATE>
...
5. **Write the spec** to `.claude/contract/specs/YYYY-MM-DD-<topic>-spec.md`.
...
8. **Ask whether to proceed with `/fb-plan`** once the user approves the spec.
9. **Stop.** Hand off to the user to run `/fb-plan <spec-file>`.
...
flowchart TD
    ...
    F -- no --> E
    F -- yes --> G[Write spec to .claude/contract/specs/]
    G --> H[Self-review: placeholders, contradictions, ambiguity, scope]
    H --> I[Ask user to review the file]
    I --> J{User approves spec?}
    J -- changes --> G
    J -- yes --> K[Ask: proceed with /fb-plan?]
    K --> L[Hand off: run /fb-plan on the spec file]
```
...
Write to `.claude/contract/specs/YYYY-MM-DD-<topic>-spec.md`. The structure is what `/fb-plan` consumes downstream.
...
## Success criteria
How the user (or `/fb-plan`) will know the change is done correctly. Observable, not subjective.

## Open questions
Anything still unresolved at the end of brainstorming. The user red-lines this section before invoking `/fb-plan`. Empty section is fine — better than fake-resolved questions.
````
...
> Spec written to `.claude/contract/specs/<file>`. Please review it and tell me if anything needs changing.
...
Once the user approves the spec, ask via `AskUserQuestion` whether to proceed with `/fb-plan`. Two options:

- **Yes, proceed** — hand off so the user can run `/fb-plan` in a fresh session.
- **Not yet** — stop here; the user will pick it up later.

Do not invoke `/fb-plan` yourself in either case — the hand-off is always the user's action.

## Hand-Off

After the user answers the proceed prompt, tell them — verbatim:

> Spec written at `.claude/contract/specs/<file>`. Run `/fb-plan .claude/contract/specs/<file>` when ready.
>
> I will stop here.

Then stop. Do not invoke `/fb-plan`. Do not call other skills. The user drives the next step.
...
- A file exists at `.claude/contract/specs/YYYY-MM-DD-<topic>-spec.md` matching the template.
...
- The user has been asked whether to proceed with `/fb-plan` and answered.
- The skill ended by handing off to the user — not by invoking `/fb-plan` or any implementation skill.
...
- "Should I commit the spec?" (no commits from this skill — proceed straight to the `/fb-plan` prompt)
- "Let me also start planning…" (the skill stops at the spec; `/fb-plan` is the user's call)
- "I'll go ahead and run `/fb-plan`." (forbidden — see Hand-Off)
...
- Invoking `/fb-plan`, implementation skills, or writing production code from this skill.
